...
interface IRLPReader {
    ...
    function payloadLocation(bytes memory item) external pure returns (
        uint payloadMemPtr,
        uint payloadLen,
        uint itemMemPtr
    );
    function numItems(bytes memory item) external pure returns (uint);
    function rlpBytesKeccak256(bytes memory item) external pure returns (bytes32);
    function payloadKeccak256(bytes memory item) external pure returns (bytes32);
    ...
    function bytesToString(bytes memory item) external pure returns (string memory);
    function toIterator(bytes memory item) external pure;
    function nestedIteration(bytes memory item) external pure returns (string memory);
