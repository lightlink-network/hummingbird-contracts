...
contract MipsChallenge {
    ...
    function initiateChallenge(
        ...
        if (
            blockNumberNHash == bytes32(0) || blockNumberNp1Hash == bytes32(0)
        ) {
            revert("block number too old to challenge");
        }
        require(
            blockNumberNp1Hash == computedBlockHash,
            "incorrect header supplied for block N+1"
        );
        ...
        bytes32 inputHash;
        {
            Lib_RLPReader.RLPItem[] memory decodedHeader = Lib_RLPReader
                .readList(blockHeaderNp1);
    ...
    function proposeState(uint256 challengeId, bytes32 stateHash) external {
        ...
        uint256 stepNumber = getStepNumber(challengeId);
        require(
            c.assertedState[stepNumber] == bytes32(0),
            "state already proposed"
        );
    ...
    function respondState(uint256 challengeId, bytes32 stateHash) external {
        ...
        require(
            c.assertedState[stepNumber] != bytes32(0),
            "challenger state not proposed"
        );
        require(
            c.defendedState[stepNumber] == bytes32(0),
            "state already proposed"
        );
    ...
    function confirmStateTransition(uint256 challengeId) external {
        ...
        require(
            stepState == c.assertedState[c.R],
            "wrong asserted state for challenger"
        );
    ...
    function denyStateTransition(uint256 challengeId) external {
        ...
        require(
            stepState == c.defendedState[c.R],
            "wrong asserted state for defender"
        );
