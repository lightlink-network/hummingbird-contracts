...
///         the L2 chain.
contract SystemConfig is OwnableUpgradeable, IGasToken {
    ...
    /// @notice Storage slot that the LightLinkPortal address is stored at.
    bytes32 public constant LIGHTLINK_PORTAL_SLOT =
        bytes32(uint256(keccak256("systemconfig.lightlinkportal")) - 1);

    /// @notice Storage slot that the OptimismMintableERC20Factory address is stored at.
    bytes32 public constant OPTIMISM_MINTABLE_ERC20_FACTORY_SLOT =
        bytes32(
            uint256(keccak256("systemconfig.optimismmintableerc20factory")) - 1
        );
    ...
    bytes32 public constant START_BLOCK_SLOT =
        bytes32(uint256(keccak256("systemconfig.startBlock")) - 1);
    ...
    ///         Used by the LightLinkPortal to meter the cost of buying L2 gas on L1.
    ...
    function initialize(
        ...
        Storage.setAddress(
            ...
            _addresses.l1StandardBridge
        );
        Storage.setAddress(
            DISPUTE_GAME_FACTORY_SLOT,
            _addresses.disputeGameFactory
        );
        Storage.setAddress(LIGHTLINK_PORTAL_SLOT, _addresses.lightlinkPortal);
        Storage.setAddress(
            OPTIMISM_MINTABLE_ERC20_FACTORY_SLOT,
            _addresses.optimismMintableERC20Factory
        );
    ...
    /// @notice Getter for the LightLinkPortal address.
    function lightlinkPortal() public view returns (address addr_) {
        addr_ = Storage.getAddress(LIGHTLINK_PORTAL_SLOT);
    }

    /// @notice Getter for the OptimismMintableERC20Factory address.
    function optimismMintableERC20Factory()
        external
        view
    ...
    function _setGasPayingToken(address _token) internal virtual {
        if (
            ...
            // Set the gas paying token in storage and in the LightLinkPortal.
            GasPayingToken.set({
                _token: _token,
            ...
            LightLinkPortal(payable(lightlinkPortal())).setGasPayingToken({
                _token: _token,
                _decimals: GAS_PAYING_TOKEN_DECIMALS,
                _name: name,
                _symbol: symbol
    ...
    function _setGasConfig(uint256 _overhead, uint256 _scalar) internal {
        require(
            (uint256(0xff) << 248) & _scalar == 0,
            "SystemConfig: scalar exceeds max."
        );
    ...
    function _setGasLimit(uint64 _gasLimit) internal {
        ...
        require(
            _gasLimit <= maximumGasLimit(),
            "SystemConfig: gas limit too high"
        );
        gasLimit = _gasLimit;
