...
contract Treasury is Ownable {
  ...
  event BondCreated(uint256 indexed id, address indexed owner, uint256 amount);
  event BondClaimed(uint256 indexed id, address indexed owner, uint256 amount);
  ...
  function claimBond(uint256 _id, uint256 _amount, address payable _beneficary) external {
    Bond storage bond = bonds[_id];
    require(bond.owner == msg.sender, "only the bond owner can claim");
    require(bond.amount >= _amount, "bond amount must be greater than amount");
  ...
  function makePayment(address payable _beneficary, uint256 _amount) public onlyOwner() returns (int256) {
  ...
  function transfer(address payable _beneficary, uint256 _amount) external payable onlyOwner() {
    _beneficary.transfer(_amount);
  }

  function transferToken(address payable _beneficary, uint256 _amount, address _token) external payable onlyOwner() {
    (bool success, ) = _token.call(abi.encodeWithSignature("transfer(address,uint256)", _beneficary, _amount));
