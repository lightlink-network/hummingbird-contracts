...
library ForgeArtifacts {
    ...
    function isInitialized(
        string memory _name,
        address _address
    ) internal returns (bool initialized_) {
        ...
        initialized_ =
            uint8((uint256(slotVal) >> (slot.offset * 8)) & 0xFF) != 0;
    }
    ...
    function getContractFunctionAbis(
        ...
        command[2] = string.concat(
            ...
            " ",
            path,
            bytes(pathExcludesPat).length > 0
                ? string.concat(" ! \\( ", pathExcludesPat, " \\)")
                : "",
            " -type f ",
            "-exec basename {} \\;",
            " | ",
        ...
        string[] memory contractNames = abi.decode(
            vm.parseJson(string(Process.run(command))),
            (string[])
        );
        ...
        for (uint256 i; i < contractNames.length; i++) {
            ...
            string[] memory methodIdentifiers = getMethodIdentifiers(
                contractName
            );
